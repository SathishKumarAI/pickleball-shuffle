...
| T3 | ✅ | **In-game pause** | Pause/Resume in the TopBar quick-actions + a full-screen "Paused" overlay that freezes the elapsed clock and blocks scoring/drawing until resumed. State persists in `GameSession` (`pausedAt`/`pausedMs`), so a paused break survives reload. Engine: `pauseGame`/`resumePlay`/`elapsedMs`/`isPaused`. |
